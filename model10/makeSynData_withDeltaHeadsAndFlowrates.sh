...
if [ -f synDataList_2.txt ]
then
	rm synDataList_2.txt
fi
...
cut -c25-263 temp.txt > gwSynHeads.txt #without the first column with times
cut -c1-23 temp.txt > assimTimes.txt  #only the first column with times
...
cp synFlowrates.txt flowrateObsData.txt

# Making one file containing all synthetic data (10 first columns of head data + 5 last ones of flowrate data)
paste -d' ' gwSynHeads.txt $(printf gwSynFlowrates%d".txt " $(seq 1 $nbLayers)) > synDataByObsPts.txt

# Only keeping the head observations
cut -c1-240 synDataByObsPts.txt > synDataByObsPts_2.txt
...
cp synHeads.txt headObsData.txt
...
cp synDeltaHeads.txt deltaHeadObsData.txt
...
cp synVerticalHeadDiffs.txt vertHeadDiffObsData.txt
