...
## Ensemble DA is here performed by the iterative ensemble smoother ES-MDA (Emerick & Reynolds 2013) in its standard version (i.e. the number of iterations is predefined by the user).
## The multiresolution multiple-point DS simulation algorithm used was developed by Straubhaar et al. (2020).
# This multiresolution simulation algorithm was integrated into a special version of DeeSse which allows hard data conditioning at the coarsest scale of the simulations.
