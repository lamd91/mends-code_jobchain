...
jobID_1=`sbatch --parsable -J genMP -o genMP.out -c 1 -p any --exclude=node[08-10]   -t 00:20:00  --array=0-${lastProcessRank} generateMPSim_savePyr.sh $modelName 0 $nbLevels`

# Make ensemble of pyramid (coarsest-level) values, categorical MPS fields
jobID_2=`sbatch --parsable -J mp-pyrEns_0 -o mp-pyrEns_0.out -n 1 -c 1 -p any --exclude=node[08-10]   -t 00:10:00 --dependency=afterok:${jobID_1} makeInitialMPSimEns.sh ${lastProcessRank}`

# Compute Normal Score Transform of pyramid values (i.e. the parameters that will be updated)
jobID_3=`sbatch --parsable -J ns_0 -o ns_0.out -c 1 --mem-per-cpu=3900 -p any --exclude=node[08-10]   -t 1:00:00 --dependency=afterok:${jobID_2} --array=0-${lastProcessRank} array_normalScoreTransform_pyr.sh $modelName 0`

# Create initial ensemble of normal score transformed values 
jobID_4=`sbatch --parsable -J nsPyrEns_0 -o nsPyrEns_0.out -n 1 -c 1 -p any --exclude=node[08-10]   -t 1:00:00 --dependency=afterok:${jobID_3} makeNSPyrEns.sh $modelName $lastProcessRank $dataTypes 0`
...
jobID_5=`sbatch --parsable -J sampleCD -o sampleCD.out -p any --exclude=node[08-10]   -c 1 -n 1 -t 00:10:00 --dependency=afterok:${jobID_4} sampleConditioningLocations.sh $modelName`

# Back Transform the transformed pyramid values
jobID_6=`sbatch --parsable -J bns_0 -o bns_0.out -c 3 --mem-per-cpu=3900 -p any --exclude=node[08-10]   -t 1:00:00 --dependency=afterok:${jobID_5} --array=0-${lastProcessRank} array_backTransform_nsPyr.sh  $modelName 0`
#jobID_6=`sbatch --parsable -J bns_0 -o bns_0.out -c 3 --mem-per-cpu=3900 -p any --exclude=node[08-10]   -t 1:00:00 --dependency=afterok:${jobID_4} --array=0-${lastProcessRank} array_backTransform_nsPyr.sh $modelName 0` # comment the lines associated to jobID_5 and jobID_6 if you uncomment this line
...
do
	...
	jobID_8a=`sbatch --parsable -J makeObsEns_${it} -o makeObsEns_${it}.out -c 1 -p any --exclude=node[08-10] -t 00:05:00 --dependency=afterok:${jobID_7} prepInflatedObsDataEns.sh $modelName $dataTypes $nbAssimilations $ensembleSize`

	jobID_8b=`sbatch --parsable -J prepObs_${it} -o prepObs_${it}.out -c 3 -p any --exclude=node[08-10] -t 00:05:00 --dependency=afterok:${jobID_8a} --array=0-${lastProcessRank} array_prepInflatedObsData.sh $modelName $dataTypes $nbAssimilations`
	...
	jobID_9=`sbatch --parsable -J condMP_${it} -o condMP_${it}.out -c 3 -p any --exclude=node[08-10] -t 01:00:00 --dependency=afterok:${jobID_8b} --array=0-${lastProcessRank} conditionMPSim.sh $modelName ${it} ${nbLevels}`
	
	# Run forward model with the updated MP simulations 
	jobID_10=`sbatch --parsable -J fwd_${it} -o fwd_${it}.out -c 1 -p any --exclude=node[08-10] -c 1 -t 01:00:00  --dependency=afterok:${jobID_9} --array=0-${lastProcessRank} array_runForwardModel.sh $modelName $dataTypes ${it} $nbAssimilations`
	...
	jobID_14=`sbatch --parsable -J calcDev_${it} -o calcDev_${it}.out -p any --exclude=node[08-10] -c 1 -t 00:10:00 --dependency=afterok:${jobID_13} --array=0-${lastProcessRank} array_calcDevFromMean.sh $ensembleSize $modelName $dataTypes`
	...
	jobID_18=`sbatch --parsable -J bns_${it} -o bns_${it}.out -c 3 --mem-per-cpu=3900 -p any --exclude=node[08-10] -t 1:00:00 --dependency=afterok:${jobID_17} --array=0-${lastProcessRank} array_backTransform_nsPyr.sh $modelName ${it}`
	...
	jobID_19=`sbatch --parsable -J uPyrEns_${it} -o uPyrEns_${it}.out -n 1 -c 1 -p any --exclude=node[08-10] -t 00:10:00 --dependency=afterok:${jobID_18} updatePyrEns.sh $modelName ${it} ${lastProcessRank}`
