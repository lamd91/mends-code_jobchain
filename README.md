...
The implementation consists in executing a "job chain" i.e. a series of jobs with dependencies, which is expressed in the file mends.sh. Note that only the SLURM commands in mends.sh should be adapted in order to run the code on a cluster using another type of scheduler (e.g. PBS, UGE, etc.)
